...
const fetchProductsByCategory = async (JinaLaKituoName) => {
  ...
  try {
    const token = await AsyncStorage.getItem('userToken');
    let url = `${EndPoint}/FilterGetNjeYaMkatabaTareheFulaniView/?page=1&page_size=500`;
    ...
    setQueryset(data.queryset || []);
    setJumlaYaWote(data.JumlaYaWote);
    setPending(false);
  } catch (error) {
    console.error(error);
    setPending(false);
...
const fetchProductsByCategory2 = async (JinaLaKituoName) => {
  ...
  try {
    const token = await AsyncStorage.getItem('userToken');
    const formattedStartDate = formatDate(startDate);
    let url = `${EndPoint}/Filter_FilterNjeYaMkatabaTareheFulaniByDate/?startDate=${formattedStartDate}&page=1&page_size=500`;
